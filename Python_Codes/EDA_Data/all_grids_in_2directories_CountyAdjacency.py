...
# %%
Supriya_FIPS = pd.read_csv(rangeland_dir + "Supriya_FIPS.csv", encoding= 'unicode_escape')
Supriya_FIPS = Supriya_FIPS[["STATEFP", "COUNTYFP", "GEOID", "NAME", "STUSPS", "NAME_1"]]
Supriya_FIPS.rename(columns=lambda x: x.lower(), inplace=True)
Supriya_FIPS.rename(columns={"name": "county",
                             "geoid":"fips",
                             "stusps": "state", 
...
for a_idx in county_adjacency.index:
    ...
    if (not math.isnan(county_adjacency.loc[a_idx, "county_geoid"])) and \
        ...
        curr_county = list(FIPS.loc[FIPS.fips==county_adjacency.loc[a_idx, "county_geoid"], "county_name"])[0]
...
# %%
county_adjacency.loc[county_adjacency.county_geoid==27111, "county_name"]=list(FIPS[FIPS.fips==27111].county_name)[0]
...
for an_ID in geo_IDs:
    if not(an_ID in county_ID_dict.keys()):
        county_ID_dict[an_ID] = county_adjacency[county_adjacency.county_geoid==an_ID].county_name.unique()[0]

# %%
neighbor_ID_dict = {}
for an_ID in neighbor_geoids:
    if not(an_ID in neighbor_ID_dict.keys()):
        neighbor_ID_dict[an_ID] = county_adjacency[county_adjacency.county_geoid==an_ID].county_name.unique()[0]
...
for an_idx in county_adjacency.index:
    ...
    a = list(FIPS[FIPS.fips==county_adjacency.loc[an_idx, "county_geoid"]]["county_name"])[0]
    ...
    b = list(FIPS[FIPS.fips==county_adjacency.loc[an_idx, "neighbor_geoid"]]["county_name"])[0]
