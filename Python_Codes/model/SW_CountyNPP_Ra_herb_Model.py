...
ratioFipsNotInData = [
    x
    for x in list(herbRatio.county_fips)
    if x not in list(county_annual_SW_Ra_cattleInv.county_fips)
]

SWDataFipsNotinRatio = [
    x
    for x in list(county_annual_SW_Ra_cattleInv.county_fips)
    if x not in list(herbRatio.county_fips)
]
...
ratioFipsNotInData = [
    x
    for x in list(herbRatio.county_fips)
    if x not in list(county_annual_NPP_Ra_cattleInv.county_fips)
]

DataFipsNotinRatio = [
    x
    for x in list(county_annual_NPP_Ra_cattleInv.county_fips)
    if x not in list(herbRatio.county_fips)
]
...
for a_county in county_annual_SW_Ra_cattleInv.county_fips.unique():
    df = county_annual_SW_Ra_cattleInv[
        county_annual_SW_Ra_cattleInv.county_fips == a_county
    ]
...
# %%
county_annual_SW_Ra_cattleInv = pd.merge(
    county_annual_SW_Ra_cattleInv, herbRatio, on=["county_fips"], how="left"
)
...
# %%
county_annual_NPP_Ra_cattleInv = pd.merge(
    county_annual_NPP_Ra_cattleInv, herbRatio, on=["county_fips"], how="left"
)
...
cnty_ann_SW_NPP_Ra = pd.merge(
    cnty_ann_SW_NPP_Ra, herbRatio, on=["county_fips"], how="left"
)
...
ax3.text(
    x=data_.min() + 30000000, y=800, s="original", fontsize=text_size, color=text_color
)
...
sns.histplot(data=data_, kde=True, bins=200, color="darkblue", ax=ax3)
ax3.text(
    x=data_.max() - 10000, y=40, s="original", fontsize=text_size, color=text_color
)

ax1.set_xlabel("")
ax2.set_xlabel("")
ax3.set_xlabel("transformed inventory")
...
ax1.set_xlabel("")
ax2.set_xlabel("")
ax3.set_xlabel("transformed herb ratio")
...
# %%
NPP_A_2017 = NPP_Ra_cattleInv_2017[
    ["county_rangeland_npp", "log_rangeland_acre", "herb_avg"]
].values
NPP_A_2017 = np.hstack([NPP_A_2017, np.ones(len(NPP_A_2017)).reshape(-1, 1)])
print(NPP_A_2017.shape)
...
NPP_A_2012 = NPP_Ra_cattleInv_2012[
    ["log_county_rangeland_npp", "log_rangeland_acre", "herb_avg"]
].copy()
